...
class DelayedEntryAgent(AgentEngine):
    ...
    def run_cycle(self, candles):
        ...
        # Jeżeli mamy oczekujący sygnał,
        # odliczamy świece do wejścia.
...
def run_delayed(candles, delay):

    agent = DelayedEntryAgent(
        config=create_config(),
        delay=delay
    )
...
def main():
    ...
    for dataset_name, path in DATASETS:
        ...
        print(
            f"{'CURRENT':>14} "
            f"{current['trades']:>8} "
            f"{current['win_rate']:>7.2f}% "
            f"{current['profit']:>12.4f} "
            f"{current['pf']:>10.3f} "
            f"{current['dd']:>12.4f}"
        )
