...
def test_backtest_engine_has_close_method():
    agent = FakeAgent()
...
def test_backtest_engine_close_returns_result():
    agent = FakeAgent()
    engine = BacktestEngine(agent)
    ...
    assert result.get_total_profit() == 5.0


def test_backtest_engine_close_clears_position():
    agent = FakeAgent()
    engine = BacktestEngine(agent)
